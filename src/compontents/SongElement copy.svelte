<script lang="ts">
    ...
    let time = 0, pixelWidth = 0, CubeDIV: any
    const cubeSpring = new Spring(180, 8, 50)
    export let rotation = cubeSpring.position
    export let updateRotation = 0
    export let isPlaying = false
    export let allowRemove = -1
    export let idleRotation = 10
    export let data: {
        id: number,
        name: string,
        description: string,
    ...
    function doFrame(milliTime: number){
        ...
        if (!time){
            time = localTime-.01
            updateRotation = localTime + updateRotation
        }
        ...
        time = localTime
        if (localTime >= updateRotation){
            cubeSpring.target = Math.sin(time * 2 + songIndex * 12) * idleRotation
            rotation = cubeSpring.update(dt)
        }
        pixelWidth = CubeDIV.clientHeight
...
<style>
    .CubeDIV {
        ...
        transform: perspective(3000px) rotateX(var(--rotation));
        margin: 20px;
    }
    ...
    .a {
        transform: perspective(3000px) translateZ(var(--width));
    }
    .b {
        transform: perspective(3000px) translateY(var(--width)) rotateX(-90deg);
    }
    .c {
        transform: perspective(3000px) translateZ(calc(var(--width) * -1)) rotateX(-180deg);
    }
    .d {
        transform: perspective(3000px) translateY(calc(var(--width) * -1)) rotateX(-270deg);
    }
</style>